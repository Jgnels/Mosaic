...
$TempRoot = Join-Path ([IO.Path]::GetTempPath()) ("mosaic-package-firewall-" + [Guid]::NewGuid().ToString("N"))
...
function New-FixturePackage {
    ...
    $Notice = Get-Content -LiteralPath (Join-Path $Root "Dagmay.RimWorld\Package\THIRD_PARTY_NOTICES.txt") -Raw
    $Entries = [ordered]@{
        "About/About.xml" = "<ModMetaData><name>Mosaic</name></ModMetaData>"
        "Assemblies/Dagmay.Core.dll" = "fixture-core"
        "Assemblies/Dagmay.Providers.dll" = "fixture-providers"
        "Assemblies/Dagmay.RimWorld.dll" = "fixture-adapter"
    ...
    try {
        ...
        try {
            foreach ($Pair in $Entries.GetEnumerator()) {
                ...
                try { $Writer.Write([string]$Pair.Value) } finally { $Writer.Dispose() }
            }
        }
        finally {
            $Archive.Dispose()
...
try {
    ...
    foreach ($Case in @($FixtureSpec.negativeCases)) {
        ...
        New-FixturePackage `
            ...
            -ReplacementContent ([string]$Case.content)
        $Rejected = $false
        try {
            $null = & $Verifier -PackagePath $CasePath -SourceRoot $Root
        }
        catch {
            if ($_.Exception.Message.IndexOf([string]$Case.expected, [StringComparison]::OrdinalIgnoreCase) -ge 0) {
                $Rejected = $true
            }
            else {
