...
$ProhibitedPathPatterns = @(
    ...
    '(^|/)\.git(/|$)',
    '(^|/)Player\.log$',
    '\.(rws|dagmay|journal|reflection|key|pfx|pem)$'
)
$UserProfilePath = [Environment]::GetFolderPath([Environment+SpecialFolder]::UserProfile)
$UserProfileLeaf = if ([string]::IsNullOrWhiteSpace($UserProfilePath)) { "" } else { Split-Path -Leaf $UserProfilePath }
    ...
    foreach ($ZipEntry in $Archive.Entries) {
        ...
        if ([IO.Path]::GetExtension($Name).ToLowerInvariant() -in @(".dll", ".pdb")) {
            ...
            if (-not [string]::IsNullOrWhiteSpace($UserProfilePath)) {
                $ContainsFullProfile =
                    $BinaryText.IndexOf($UserProfilePath, [StringComparison]::OrdinalIgnoreCase) -ge 0 -or
                    $BinaryText.IndexOf($UserProfilePath.Replace('\', '/'), [StringComparison]::OrdinalIgnoreCase) -ge 0
            }
            $ContainsSegmentedProfile = $false
            if (-not [string]::IsNullOrWhiteSpace($UserProfileLeaf)) {
                $ContainsProfileLeaf = $BinaryText.IndexOf($UserProfileLeaf, [StringComparison]::OrdinalIgnoreCase) -ge 0
                $ContainsProfileContainer =
                    $BinaryText.IndexOf("Users", [StringComparison]::OrdinalIgnoreCase) -ge 0 -or
                    $BinaryText.IndexOf("AppData", [StringComparison]::OrdinalIgnoreCase) -ge 0 -or
