...
namespace Dagmay.Core.Contracts
{
    ...
    public readonly struct FactId : IEquatable<FactId>
    {
        ...
        {
            if (value == Guid.Empty) throw new ArgumentException("Fact ID cannot be empty.", nameof(value));
            Value = value;
        }

        public Guid Value { get; }
        public static FactId New() => new FactId(Guid.NewGuid());
        public static FactId Parse(string value) => new FactId(Guid.Parse(value));
