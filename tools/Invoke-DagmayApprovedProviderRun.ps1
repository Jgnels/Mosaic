[CmdletBinding()]
param(
    [ValidateSet("HARDENED_PROVIDER_SMOKE_V1", "MOSAIC_MEMORY_GROUNDING_PILOT_V1", "MOSAIC_RELATIONSHIP_BALANCE_PILOT_V1", "MOSAIC_RELATIONSHIP_BALANCE_PILOT_V2", "MOSAIC_TYPED_RELATIONSHIP_PILOT_V3")]
...
Invoke-WithDagmayGeminiCredential -ScriptBlock {
    if ($Protocol -eq "MOSAIC_TYPED_RELATIONSHIP_PILOT_V3") {
        & $Python (Join-Path $repoRoot "syntheticlab\run_mosaic_typed_relationship_pilot_v3.py") `
            --approval $approvalPath --output $output --checkpoint $checkpoint `
            --ledger $ledger --payload-archive $archive
    }
    elseif ($Protocol -eq "MOSAIC_RELATIONSHIP_BALANCE_PILOT_V2") {
        & $Python (Join-Path $repoRoot "syntheticlab\run_mosaic_relationship_balance_pilot_v2.py") `
            --approval $approvalPath --output $output --checkpoint $checkpoint `
            --ledger $ledger --payload-archive $archive
    }
    elseif ($Protocol -eq "MOSAIC_RELATIONSHIP_BALANCE_PILOT_V1") {
