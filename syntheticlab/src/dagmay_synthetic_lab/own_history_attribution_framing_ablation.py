...
def run_own_history_attribution_framing_ablation(
    *,
    checkpoint_path,
    revision_result: dict,
    lived_history_payload: dict,
...
def _analyze_neutral_own_history_base(
    *,
    payload: dict,
...
def analyze_own_history_attribution_framing(
    *,
    payload: dict,
    baseline_analysis: dict,
    v27_analysis: dict,
    own_history_analysis: dict,
    foreign_history_analysis: dict,
    permutations: int = 20000,
    seed: int = 3101,
) -> dict:
    base = _analyze_neutral_own_history_base(
        payload=payload,
    ...
    return {
        "experiment_id": (
            "SL-OWN-HISTORY-ATTRIBUTION-FRAMING-ABLATION-ANALYSIS-001"
        ),
        "condition": (
            "OWN_HISTORY_CONTENT_SOURCE_RELATION_WITHHELD"
        ),
        "neutral_own_content_result": (
            base
        ),
        "true_own_history_reference": {
            "between_branch_centroid_distance": (
                own_between
            ),
            "signal_to_trajectory_variance_ratio": (
                own_ratio
            ),
            "permutation_p_value": (
                own_p
        ...
        "crossed_foreign_history_reference": {
            "focal_branch_permutation_p_value": (
                foreign_focal_p
            ),
            "reference_source_permutation_p_value": (
                foreign_source_p
            ),
            "status": (
                foreign_history_analysis[
        ...
        "neutral_vs_true_own_between_distance_ratio": (
            relative_between
        ),
        "neutral_vs_true_own_signal_variance_ratio": (
            relative_ratio
        ),
        "attribution_framing_status": (
            status
        ),
        "canonical_self_models_mutated": (
            False
        ...
        "source_relation_withheld_not_falsified": (
            True
        ),
        "interpretation": (
            "The exact own-history content is preserved, but the provider is not told "
            "that the archived material belongs to the focal individual and is told only "
            "that source relation is withheld. This distinguishes semantic content matching "
            "from explicit autobiographical ownership attribution without falsely assigning "
            "another branch's history as self-history."
        ),
    }
